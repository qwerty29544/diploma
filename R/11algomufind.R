...
mu3 <- function(z1, z2, z3) {
    1i * (Mod(z1)^2 * (z2 - z3) + Mod(z2)^2 * (z3 - z1) + Mod(z3)^2 * (z1 - z2)) / (2 * Im(z1 * Conj(z2) + z2 * Conj(z3) + z3 * Conj(z1)))
}
...
#'
#' @examples
GMSI <- function(A = matrix(), f = numeric(), u = numeric(), eps = 10e-3) {
...
#'
#' @examples
GMSI.history <- function(A = matrix(), f = numeric(), u = numeric(), eps = 10e-3) {
...
GDM <- function(A = matrix(), f = numeric(), u = numeric(), eps = 10e-4) {
    ...
    if ( abs((t(r) %*% (A %*% t(A) %*% r)))^2 / ((t(r) %*% r) * (t(A %*% t(A) %*% r)^2 %*% (A %*% t(A) %*% r)^2)) >= 1) {
        warning("Iterations do not sovling operator equation")
        return(NULL)
    }
    repeat{
        ut <- u
...
IMRES <- function(A = matrix(), f = numeric(), u = numeric(), eps = 10e-4) {
    if (dim(A)[1] != dim(A)[2]) {
        warning("Operator must be a quadratic")
        return(NULL)
    }
    
    h <- A %*% u - f
    if ((1 - ((t(h) %*%  (A %*% h))^2 / ((t(h) %*% h) * (t(A %*% h) %*% (A %*% h))))) < 0) {
        warning("q >= 1, method is growing")
        return(NULL)
    }
...
Jacobi <- function(A = matrix(), f = numeric(), u = numeric(), eps = 10e-4) {
    ...
    if (!all.equal(diag(A), apply(A, 1, max))) {
        warning("Operator must have a diagonal priority!")
        return(NULL)
    }
